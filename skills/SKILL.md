...
1. Have them run `zanei start` first (`zanei stop && zanei start` if it is already running). The recorder asks macOS for required Accessibility and Input Monitoring permissions at startup. Exit code 3 means the recorder started but reported missing permissions; it remains running.
...
4. Run `zanei stop && zanei start` so the recorder picks up the granted permissions.
