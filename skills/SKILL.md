...
1. Have them run `zanei start` first (`zanei stop && zanei start` if it is already running). The recorder requests Accessibility first and Input Monitoring after it. The Input Monitoring dialog may follow, but often does not appear; in that case, use `zanei doctor --fix` as described below to copy and reveal the exact app path for manual addition. Exit code 3 means the recorder started but reported missing permissions; it remains running.
