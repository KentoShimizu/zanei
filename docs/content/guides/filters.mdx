...
One startup-time exception: whether the recorder talks to Chrome at all (and therefore whether Automation is required) is decided when recording starts. If Chrome was excluded from the content-snapshot scope at that moment and you include it later, content snapshots for Chrome start after `zanei stop && zanei start`; every other scope change applies within a few seconds.
