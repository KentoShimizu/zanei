...
A current reason is removed only when the collector reports actual recovery. A reason retained after an unexpected collector exit is removed after the restarted collector runs stably for 60 seconds. Recovery does not reset `collector_failures`, so an empty degraded map and a nonzero cumulative count is normal.
