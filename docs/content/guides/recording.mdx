...
`pause` and `resume` act on a running recorder. To start one that is already paused — so nothing is
recorded between the start and the pause — use `zanei start --paused`. The pause lives in the store,
so `zanei status --json` reports it under `paused_persisted` even while no recorder runs.
