...
- Without `browser`, Automation for a supported browser is still required when that browser is allowed by the text-content scope with `text_content` and `ui` or `input`, or by an enabled content-snapshot scope. A browser excluded from all relevant scopes does not require Automation.
