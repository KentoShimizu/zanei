...
The recorder stops creating snapshots after 128 MiB in a day. With the default 48-hour retention, that guard bounds snapshot text to about 256 MiB before database overhead; normal use is expected to be much lower because identical snapshots are not stored.
...
- **Any process that can run `zanei` as you.** Encryption protects copies of the file, not the CLI. Anything running as your user — including an agent with shell access, which can run `zanei export` — can read the data, as can root or anything that reads process memory. Theft of the disk itself is covered by FileVault, not by Zanei.
