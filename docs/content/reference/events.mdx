...
| --- | --- | --- |
| 64 KiB (65,536 bytes) | `element.value`; `data.text` for `ui.value`, `input.key`, `clipboard.copy`, and `clipboard.paste` | Keeps useful text and clipboard differences while bounding per-field redaction, serialization, and in-memory batch cost |
...
The published JSON Schema is authoritative for `v: 1`. Its envelope, event taxonomy, contexts, and per-type payloads are closed: unknown fields and event types are rejected. Adding a field or event type requires a new `v` and a matching store migration.
