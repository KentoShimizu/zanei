...
`store_key` reports where the store's encryption key is. `state` is `keychain` (found in the login Keychain), `file` (read from the `ZANEI_STORE_KEY_FILE` development override), `not_needed` (the store is missing, plaintext, or not recognized), `missing` (the store is encrypted but there is no key item), `mismatch` (the key does not decrypt this store), `keychain_locked`, `keychain_denied`, or `unavailable`; `detail` is present only when there is more to report. Human-readable output adds a `Store key: ...` line after the permission table. `doctor` does not fail when the store is locked: it reports the key state and probes permissions from its own process instead.
...
On its first start, the recorder generates the store's encryption key and saves it in your login Keychain as "Zanei store key". A plaintext store written by an earlier version is converted in place on the first start after upgrading. The recorder never shows Keychain dialogs: if the login Keychain is locked, `start` fails with `store is locked: your login Keychain is locked; unlock it (for example by opening Keychain Access) and try again`.
...
| `store.encryption` | `"sqlcipher"` \| `"plaintext"` \| null | `sqlcipher` for an encrypted store; `plaintext` for a store written before encryption existed, which the recorder converts on its next start; null when the store is missing or unreadable |
...
When the store does not exist, `purge` prints `Purged 0 events` and does not create one.
