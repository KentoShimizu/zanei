...
| Command | Summary |
| --- | --- |
| [`doctor`](#doctor) | Diagnose macOS permissions and recorder health; guide granting |
...
zanei start --paused     # start with recording suspended until `zanei resume`
```

| Flag | Description |
| --- | --- |
| `--foreground` | Don't daemonize; run in the foreground |
| `--paused` | Bring the recorder up with recording suspended; lift it with [`resume`](#resume) |

`--paused` records the pause in the store before the recorder process exists, so there is no
window between the start and a later [`pause`](#pause) in which events are captured. The recorder
reads the request in the same startup that claims the store lock and comes up with every collector
stopped. It works with `--foreground` as well, and creates the store (and its encryption key) when
they do not exist yet.

The store is the only source of truth for the pause: `--paused` is not remembered by the launch
agent, so a recorder launchd restarts after a crash honors whatever the store says at that moment,
and [`resume`](#resume) lifts a `--paused` pause like any other. The request is indefinite — the
strictest pause `pause` can make — so a pause already in the store is never weakened by it.
`--paused` fails with **code 1** when another recorder already owns the store, without changing
that recorder's pause state.
...
`pause` and `resume` need a running recorder and exit with **code 4** when there is none. To bring
a recorder up already paused, use [`start --paused`](#start); to learn the store's pause without
starting a recorder, read `paused_persisted` from [`status --json`](#status).
...
| `paused` | boolean \| null | Whether recording is suspended right now. For a readable store, true only when a lock owner exists, its heartbeat is fresh, and its pause request is active. A stale, future, or missing heartbeat and a readable stopped store report false. Null when database contents cannot be read |
| `paused_persisted` | boolean \| null | Whether the store carries an active pause request, whether or not a recorder runs: the state the next recorder to open this store comes up in. A timed pause that has expired reads false. Null when database contents cannot be read |
...
| `STORED PAUSE` | `paused_persisted`, or `-` when null |
