...
Configures Zanei instructions and MCP integration for the target agent. Agents with native skill support and Claude Desktop retain their file-based setup; opencode only prints manual setup instructions and does not modify files.
...
| `--scope` | `project` (default; current repository) / `user` (account-wide). It has no effect on the manual opencode output |
| `--print` | Preview planned file changes without writing. opencode does not write files, with or without this flag |
