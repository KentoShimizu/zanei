...
| `3` | **Missing permissions**. For background `start`, the recorder is already running and reported the missing TCC permission through its heartbeat; `doctor` uses the same code for its diagnostic result |
...
After a background recorder reports that required permissions are missing, `start` prints the same granting guidance as `doctor` and exits with **code 3**. The recorder remains running; grant the permissions, run `zanei stop && zanei start`, and confirm with `zanei doctor`. `--foreground` runs the same recorder startup request logic in the foreground process.
