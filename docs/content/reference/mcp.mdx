...
ChatGPT is not one of them. Its connectors accept only remote HTTPS MCP servers, while `zanei mcp` is a local stdio process that opens no network connection of its own. Bridging it to a public URL would put the store on the network, so use [Codex](/agents/setup) for OpenAI agents. What an agent does with the results it reads is a separate question — see [security boundary](#security-boundary).
