...
`zanei mcp` exposes recorded activity to MCP clients: terminal agents (Claude Code, Codex, opencode, Hermes Agent) and chat clients that cannot run the CLI (Claude Desktop, ChatGPT connectors), for which MCP is the integration path.
