...
## I opted in, but typed text from Slack / Claude / VS Code is missing

Electron and other Chromium-based apps build their accessibility tree lazily, so their input
fields often report as *unknown* to macOS Accessibility. Zanei refuses to capture text from a
field it cannot classify — the same safeguard that keeps password fields out — so typed
differences from those apps are absent by design. Text you *copy* there is still captured
(clipboard events don't depend on field classification), and everything else about the app —
focus, titles, clicks, the fact of typing — is recorded normally. A future release may activate
the accessibility tree for these apps to make their typed text capturable.
