...
Stores written by earlier versions are plaintext SQLite. On the first `zanei start` after upgrading, the recorder converts the existing store in place: it writes the encrypted copy next to the old file and swaps it in atomically, so a crash leaves either the old file or the new one. The recorder logs `zanei: encrypted the existing store (N events)`. Until then, `zanei query` and the other readers still read the plaintext store, and the `STORE` line of `zanei status` ends with `(plaintext; the recorder encrypts it on its next start)`.

The old plaintext blocks may remain in the disk's free space until they are overwritten; FileVault covers that. To leave nothing behind, run `zanei purge --all` before upgrading.
