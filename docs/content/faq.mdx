...
No. Keystroke content is not captured unless you opt in (`text_content = true`); by default only the fact of typing and the field type are recorded. The first fully-granted interactive `start` asks once whether to opt in, defaulting to no, and `zanei config set capture.text_content <true|false>` changes it later. Password fields are excluded even if you opt in, redactors scrub recognizable secrets, and data does not leave your machine. See the [privacy model](/guides/privacy).
...
typed text is not captured; text you copy there, and the fact of typing, are still recorded.
...
Text content stays off unless you opted in at first start. Check `zanei status`; if `TEXT CONTENT` is off, enable it explicitly and restart recording:
