...
Electron apps build their accessibility tree lazily, so their input fields can otherwise report as
*unknown* to macOS Accessibility. With `capture.text_content` enabled, Zanei requests the tree when
it attaches its observer so eligible typed text can be captured. Zanei still refuses to capture
text from any field it cannot classify, preserving the same safeguard that keeps password fields
out. Restart Zanei after changing `capture.text_content`.
