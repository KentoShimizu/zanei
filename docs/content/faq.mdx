...
A 0.2.x binary cannot open the schema-7 live store, and its strict config reader can reject `capture.content_snapshot`, `[filter.text_content]`, and `[filter.content_snapshot]`. Do not change the store metadata back to version 5. To return to 0.2.x, restore a pre-upgrade backup and remove those settings; otherwise keep a 0.3.x binary available to read the new store.
