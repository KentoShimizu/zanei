...
Some Chromium-based apps do not expose an accessibility content tree to external clients; ChatGPT
desktop was one observed example in August 2026 testing. If an app does not expose that tree,
typed-text bodies (`ui.value` / `data.text`) and content snapshots (`content.snapshot`) for that
content are missing, and Zanei does not use screen images as a substitute. Keystroke facts (such as
