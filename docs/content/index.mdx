...
Zanei currently runs on **macOS only** (Apple Silicon and Intel).
:::
...
This returns the last two hours of your activity as a Markdown timeline, split into sessions and deduplicated. Coding agents like Claude Code or Codex CLI can read it to understand what you were just working on.

## What you can do with it

<CardGroup cols={2}>
  <Card title="Resume work" icon="history">
    Tell your agent to pick up where you left off. It reads the timeline to find the PR you were reviewing, the files you were editing, and the pages you were reading.
  </Card>
  ...
  <Card title="Context for agents" icon="bot">
    Zanei connects to major agents over the CLI and MCP, with shipped skill files. Chat clients that cannot run the CLI, such as Claude Desktop, are covered over MCP.
  </Card>
</CardGroup>
...
| --- | --- |
| **Local-only, encrypted at rest** | There is no network path for sending your data anywhere. The store file is encrypted; the key stays in your login Keychain. |
...
Zanei captures OS events such as app switches, window focus, UI interactions, and browser URL changes, normalizes them, applies privacy processing, and writes them to a **local SQLite store**. Everything that reads data back (CLI, MCP) is a thin wrapper over the same store.

## Two interfaces

| Interface | Role |
| --- | --- |
| [CLI](/reference/cli) | The core surface. A single `zanei` binary handles recording control, permission diagnostics, timeline retrieval, and filter management. |
| [MCP server](/reference/mcp) | A read-only view over the same store. The only path for chat clients that cannot run the CLI, such as Claude Desktop. |
...
- Browser URL capture supports **Google Chrome**, with **Safari** added in the unreleased development version. See the [event reference](/reference/events) for the reasoning and per-browser status.
