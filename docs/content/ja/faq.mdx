...
単体利用・組み込み利用の両方でChromeとSafariに対応します。Chromeのwindow modeでincognitoのURLイベントと本文を抑止します。Safariのプライベート状態は不明として扱い、プライベート閲覧の除外は保証しません。FirefoxのURL取得は未実装です。詳細とブラウザ別の表は[イベントリファレンス](/ja/reference/events#ブラウザの-url-捕捉)へ。
