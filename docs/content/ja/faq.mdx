...
0.2.x binary は schema 7 の live store を開けず、strict な config reader は `capture.content_snapshot`・`[filter.text_content]`・`[filter.content_snapshot]` を拒否し得ます。store metadata を version 5 に書き戻してはいけません。0.2.x に戻すには upgrade 前の backup を restore し、これらの設定を除いてください。それ以外は新しい store を読むために 0.3.x binary を残します。
