...
Electron アプリはアクセシビリティツリーを遅延構築するため、入力欄が macOS
Accessibility からは「種類不明」に見えることがあります。`capture.text_content` が有効な
場合、Zanei は observer の attach 時にツリー構築を要求し、条件を満たす打鍵本文を捕捉できる
ようにします。種類を確認できない欄の本文は引き続き記録しないため、パスワード欄を誤って
拾わない安全装置は維持されます。設定変更後は Zanei を再起動してください。
