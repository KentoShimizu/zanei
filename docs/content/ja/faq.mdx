...
## opt-in したのに Slack / Claude / VS Code で打った本文が残りません

Electron などの Chromium 系アプリはアクセシビリティツリーを遅延構築するため、入力欄が
macOS Accessibility からは「種類不明」に見えることがよくあります。Zanei は種類を確認でき
ない欄の本文を記録しません — パスワード欄を誤って拾わないための同じ安全装置です。その
ため、これらのアプリで打った差分は設計上残りません。**コピーした本文は残ります**（クリ
ップボードの記録は欄の分類に依存しません）。フォーカス・タイトル・クリック・入力の事実
など本文以外は通常どおり記録されます。将来のリリースで、これらのアプリのアクセシビリテ
ィツリーを有効化して打鍵本文も取れるようにすることを検討しています。
