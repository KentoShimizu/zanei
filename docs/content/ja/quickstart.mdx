...
`-` はローカル利用向けの ad-hoc 署名 app を作るため、再ビルド後に macOS が権限の
再付与を求めることがあります。再ビルド後も権限を維持するには、永続的な開発用証明書の
identity を `make-app.sh` に渡してください。詳細は
[packaging/README.md](https://github.com/KentoShimizu/zanei/blob/main/packaging/README.md)を参照してください。

素の `cargo install` バイナリは配布用 app bundle ではありません。`+` で追加した権限の行が
定着しないことがあり、`dev.zanei.recorder` 宛ての reset も適用されません。安定して権限を
管理するには app bundle 版を使用してください。
:::
