...
起動時だけの例外が 1 つあります。recorder が Chrome と通信するかどうか（したがって Automation が必要かどうか）は記録開始時に決まります。その時点で Chrome が content snapshot の scope から除外されていて、あとから含めた場合、Chrome の content snapshot は `zanei restart` の後に始まります。それ以外の scope 変更は数秒以内に反映されます。app mutation によって Chrome を対象へ加え、現在の topology が Chrome window を追跡していない可能性がある場合、CLI はこの restart note を表示します。
