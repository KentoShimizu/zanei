...
- `browser` がない場合も、対象browserがtext-content scopeで許可され、`text_content` と `ui` / `input` が有効な場合、または有効なcontent-snapshot scopeで許可される場合は、そのbrowserへのAutomation権限が必要です。関連scopeがすべて除外するbrowserでは不要です。Safari対応は未リリースの開発版に含まれます。
