...
**Zanei**（残映・ざんえい）は、あなたが PC で「何をしていたか」を OS のイベントから時系列に記録し、AI agent がそのまま読める形で取り出せるようにするツールです。

:::note[対応プラットフォーム]
Zanei は現在 **macOS のみ**対応です（Apple Silicon / Intel）。Windows・Linux はロードマップ上にあります。
:::
...
<CardGroup cols={2}>
  ...
  <Card title="agent の文脈強化" icon="bot">
...
| [MCP サーバ](/ja/reference/mcp) | 同じストアへの read-only ビュー。シェルを持たない MCP クライアント（Claude Desktop 等）への唯一の経路です。 |
