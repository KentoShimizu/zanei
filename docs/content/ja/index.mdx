...
<CardGroup cols={2}>
  ...
  <Card title="agent に文脈を渡す" icon="bot">
    CLI と MCP の 2 つの経路と同梱の skill で主要 agent につながります。CLI を実行できない chat クライアント（Claude Desktop など）へは MCP で届きます。
  </Card>
</CardGroup>
...
| [MCP サーバ](/ja/reference/mcp) | 同じストアへの read-only ビュー。CLI を実行できない chat クライアント（Claude Desktop 等）にとっては唯一の経路です。 |
...
- ブラウザURLの記録は**Google Chrome**に対応し、未リリースの開発版では**Safari**にも対応します。理由と各ブラウザの扱いは[イベントリファレンス](/ja/reference/events)を参照してください。
