...
| `[filter.capture_policy]` | 任意。既定では存在しません。テーブルを書いた場合は `browser` と `ide`、およびその中の全キーが必須で、`allowed_apps` だけが任意です。[`[filter.capture_policy]`](#filtercapture_policy) を参照 |
## `[capture]`

| キー | 既定値 | 説明 |
| --- | --- | --- |
| `sources` | `["app", "window", "ui", "input", "browser"]` | 捕捉するイベントファミリー。`browser` がない場合、browserのAutomationは、対象のcontent snapshot、または `text_content` と `ui` / `input` を有効にした場合に必要です。`window` 単独では不要です（[権限](/ja/guides/permissions)参照） |
...
[`zanei filter exclude-app`](/ja/reference/cli#filter) と `zanei filter only-app` が管理する list が
...
2. `allowed_apps` がある場合、アプリの表示名が含まれていなければ拒否します（前後の whitespace を除いた
   大文字小文字を無視した一致）。
...
| `browser.block_auth` | 必須 | host の先頭ラベル・path・fragment のいずれかに、代表的な sign-in・sign-up・OAuth・パスワードリセットの segment を持つ URL を拒否します。判定前に末尾の拡張子を 1 つ取り除くため、`/login.html` は `/login` として扱います。URL パターンであって認証画面の検出ではないため、`/docs/oauth` やアンカー `#login` も一致し、`/login-guide` は一致しません |
| `browser.block_payments` | 必須 | 同じ 3 か所を同じ規則で見て、`checkout`・`payment`・`payments`・`billing`・`subscription`・`subscriptions` を持つ URL を拒否します。`checkout.vendor.example` は拒否し、`/checkout-history` は拒否しません |
