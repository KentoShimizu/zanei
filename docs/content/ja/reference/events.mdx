...
| フィールド | 説明 |
| --- | --- |
| `v` | この event type を解釈するために必要な最小 envelope version。既存 type は `1`、`content.snapshot` は `2` |
...
| 上限 | 対象 field | 根拠 |
| --- | --- | --- |
| 32 KiB（32,768 bytes） | 取得時の `content.snapshot.data.text` | 有用な可視 text を保ちながら Accessibility snapshot 1 件を制限。打ち切った snapshot は `data.complete: false` |
...
| `browser.navigate` | `macos.applescript` | URL / タブの変化。**現在は Chrome のみ**。incognito は常に除外 | アクセシビリティ + オートメーション |
| `clipboard.copy` / `clipboard.paste` | `macos.eventtap` | クリップボード操作。内容は opt-in | アクセシビリティ + 入力監視 |
| `content.snapshot` | `macos.ax` | 前面ウィンドウの可視範囲に表示された Accessibility text。専用の opt-in | アクセシビリティ |

`--types` フィルタでは末尾ワイルドカードでファミリー全体を選べます：`browser.*`・`ui.*`・`content.*` など。
...
| `content.snapshot` | ✓ | — | `text`（core size rule 適用後は string または null）、`chars`（collector 上限適用後・redaction 前の文字数）、`complete`（可視 tree を取得上限内で読み終えたか）、`trigger`（`settle` / `refresh` / `focus_out`）。source は `macos.ax` |
...
envelope version は出力全体ではなく event type ごとに決まります。既存 13 type は `v: 1`、`content.snapshot` は `v: 2` が必要です。`v: 1` だけを理解する consumer は `v: 2` event を受理する必要がありません。envelope・event taxonomy・context・型別 payload は closed です。`content.snapshot` の `v: 1`、既存 type の `v: 2`、未知の field・event type は不正です。type の追加には新しい最小 `v` と対応する store migration が必要です。
