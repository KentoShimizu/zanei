...
| `browser.navigate` | `macos.applescript` | URL / タブの変化。ChromeとSafari（未リリース）に対応。Chrome incognitoは除外 | アクセシビリティ + オートメーション |
...
`browser.navigate`（URL 捕捉）はChromeに対応し、未リリースの開発版ではSafariにも対応します。単体利用と組み込み利用で同じブラウザ収集経路を使います。Chromium は AppleScript でウィンドウのモード（`normal` / `incognito`）を返します。incognito は URL イベントを生成せず、本文も設定なしで null のままです。共有イベント契約では、プライバシーモードを判定できない観測を `mode: "unknown"` で表せます。この観測を `normal` として扱うことはありません。
...
| **Safari** | ✅ 未リリースの開発版で対応 | `mode: "unknown"`。プライベート閲覧の除外は保証しない |
