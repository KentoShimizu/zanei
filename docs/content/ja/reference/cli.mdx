...
| `1` | 一般エラー |
| `2` | 使用法エラー（引数不正） |
| `3` | **権限不足**。バックグラウンド `start` では recorder は起動済みで、heartbeat を通じて不足権限を報告しています。`doctor` も診断結果に同じコードを使います |
...
バックグラウンド recorder が必要権限の不足を報告すると、`start` は `doctor` と同じ付与手順を表示し、**終了コード 3** で終了します。recorder は動作を継続します。権限を付与し、`zanei stop && zanei start` してから `zanei doctor` で確認してください。`--foreground` も同じ recorder 起動時の要求ロジックを前面プロセスで実行します。
