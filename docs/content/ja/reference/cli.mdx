...
`store_key` はストアの暗号化鍵の所在を報告します。`state` は `keychain`（ログインキーチェーンで見つかった）、`file`（開発用上書き `ZANEI_STORE_KEY_FILE` から読んだ）、`not_needed`（ストアがない・平文・認識できない）、`missing`（ストアは暗号化されているが鍵の item がない）、`mismatch`（鍵でこのストアを復号できない）、`keychain_locked`、`keychain_denied`、`unavailable` のいずれかです。`detail` は補足がある場合だけ含まれます。人間向け出力では権限の表の後に `Store key: ...` の行が加わります。ストアがロックされていても `doctor` は失敗せず、鍵の状態を報告し、権限は自身のプロセスから probe します。
...
recorder は初回起動時にストアの暗号化鍵を生成し、「Zanei store key」としてログインキーチェーンに保存します。以前のバージョンが書いた平文のストアは、アップグレード後の初回起動でその場で変換されます。recorder はキーチェーンのダイアログを表示しないため、ログインキーチェーンがロックされていると `start` は `store is locked: your login Keychain is locked; unlock it (for example by opening Keychain Access) and try again` で失敗します。
...
| `store.encryption` | `"sqlcipher"` \| `"plaintext"` \| null | 暗号化されたストアは `sqlcipher`。暗号化以前に書かれたストアは `plaintext` で、recorder が次回起動時に変換する。ストアがない、または読めない場合は null |
...
store が存在しない場合、`purge` は `Purged 0 events` を表示し、store を作成しません。
