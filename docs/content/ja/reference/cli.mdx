...
zanei start --paused     # 記録を停止した状態で起動し、`zanei resume` まで維持
```

| フラグ | 説明 |
| --- | --- |
| `--foreground` | デーモン化せず前面で実行 |
| `--paused` | 記録を停止した状態で recorder を起動する。解除は [`resume`](#resume) |

`--paused` は recorder プロセスが存在するより前に pause をストアへ記録するため、起動から後続の
[`pause`](#pause) までの間にイベントが捕捉される窓はありません。recorder は store lock を獲得する
のと同じ起動処理でこの要求を読み、すべての collector を停止した状態で立ち上がります。
`--foreground` と併用でき、ストア（および暗号鍵）が存在しない場合は作成します。

pause の唯一の source of truth はストアです。`--paused` は launch agent に記憶されないため、
クラッシュ後に launchd が再起動した recorder はその時点のストアの内容に従い、
[`resume`](#resume) は `--paused` による pause も通常どおり解除します。要求は無期限 — `pause` が
行える最も強い pause — であるため、ストアに既にある pause が弱められることはありません。
他の recorder が既にストアを所有している場合、`--paused` はその recorder の pause 状態を変更せず
**終了コード 1** で失敗します。
...
`pause` と `resume` は動作中の recorder を必要とし、存在しなければ **終了コード 4** で終了します。
停止した状態で recorder を起動するには [`start --paused`](#start) を使用し、recorder を起動せずに
ストアの pause を知るには [`status --json`](#status) の `paused_persisted` を読みます。
...
| `paused` | boolean \| null | 現在記録が停止しているか。store を読める場合、lock owner が存在し、その heartbeat が fresh で pause request が有効な場合だけ true。stale・future・missing heartbeat および読める stopped store は false。database 内容を読めない場合は null |
| `paused_persisted` | boolean \| null | recorder の動作有無にかかわらず、store が有効な pause request を保持しているか。次にこの store を開く recorder が起動する状態を表す。期限切れの時限 pause は false。database 内容を読めない場合は null |
...
| `STORED PAUSE` | `paused_persisted`。null は `-` |
| `SINCE` | `since`。null は `-` |
...
| `exclude-site` | `exclude_websites` | 指定 Chrome/Safari URL host を除外。両 mode で有効 |
| `only-site` | `include_only_websites` | 非空なら指定 Chrome/Safari URL host だけを含める |
...
- サイトルールはChromeとSafariに適用します。変更は次の設定監視 cycle（通常 2 秒以内）で反映され、再起動は不要です。
...
| `--scope` | `project`（既定。カレントリポジトリ）/ `user`（ユーザー全体） |
