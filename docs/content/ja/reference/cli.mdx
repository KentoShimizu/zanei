...
\g<0>| `store.retired_plaintext` | string[] | 暗号化へのアップグレード時に退避した平文ストアのうち、読み取りがまだ含めているもの（古い順）。保持期間を外れると空になります。人間向け出力では `PREVIOUS STORE` 行として表示。読めないものはスキップし `degraded.retired_store` に報告します |
