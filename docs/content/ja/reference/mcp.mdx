...
`zanei mcp` は記録された行動履歴を MCP クライアントへ公開します。対象は、ターミナル型 agent（Claude Code・Codex・opencode・Hermes Agent）と、CLI を実行できない chat クライアント（Claude Desktop・ChatGPT コネクタ）です。
