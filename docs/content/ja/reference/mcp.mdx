...
`limit` を超えた場合は `truncated: true`。`skipped_unknown_types` は、この binary が type を認識できず skip した store row の件数です。イベント内容は捕捉時点で既にプライバシー処理（secure field 除外・redaction）済みのものが格納されています — MCP 層で除外済みの情報が復元されることはありません。
